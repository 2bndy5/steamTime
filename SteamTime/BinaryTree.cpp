...
treeNode * BinaryTree::findParent(unsigned long long numberToDelete, treeNode * curr)
{
	...
	if (curr->leftPointer->user->id_64 == numberToDelete)
		return curr;
	else if (curr->rightPointer->user->id_64 == numberToDelete)
		return curr;
	else if (numberToDelete > curr->user->id_64 && curr->leftPointer != NULL)
		return findParent(numberToDelete, curr->leftPointer);
	else if (numberToDelete < curr->user->id_64 && curr->rightPointer != NULL)
...
bool BinaryTree::insertNode(treeNode * tempPtr)
{
	...
	while (!success)
	{
		if (curr->user->id_64 > tempPtr->user->id_64)
		{
		...
		else if (curr->user->id_64 < tempPtr->user->id_64)
		{
...
* INPUT PARAMETERS: treeNode->user->id_64 to delete ( checks if node 
...
void BinaryTree::deleteNode(unsigned long long numberToDelete)
{
	...
	if (root->user->id_64 != numberToDelete) {
		...
		if (Parent != NULL)//NULL means could not find node
		{
			...
			else if (numberToDelete < Parent->user->id_64) {
